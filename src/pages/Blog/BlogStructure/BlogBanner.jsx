...
import AjInputBase from "../../../components/AjTypography";
import { user_icon } from '../../../styles/AppConstant';
import { blogCSS } from '../blogStyles';
import { convertFromRaw } from "draft-js";
import { convertToHTML } from "draft-convert";


const BlogBanner = ({bannerInfo, filterBlog}) => {
    ...
    <Box sx={{ ...blogCSS.banner }}>
        <Box sx={{ ...blogCSS.bannerTop }}>
            <Box sx={{ ...blogCSS.bannerTopTitle }}>
                ...
                <Typography sx={{ ...blogCSS.general_para }}>A center for all our resources and insights</Typography>
            </Box>
            <Box>
                <AjInputBase
                    name="search by name"
          ...
          <Box sx={{ ...blogCSS.dimension, ...blogCSS.b_below }}>
                <Box sx={{ ...blogCSS.info, ...blogCSS.info_top }}>
                    ...
                    <ul style={{ ...(blogCSS.info_min) }}>
                        ...
                        <li>{bannerInfo?.reading_minutes} {bannerInfo?.reading_minutes > 1 ?  "mins" : "min"}</li>
                    </ul>
                </Box>
                <Typography sx={{ ...blogCSS.title_b }}>{bannerInfo?.title}</Typography>
                ...
                <Link to={`/blog/${bannerInfo?.id}`} style={{ ...(blogCSS.read_more_top_btn), textDecoration: "none" }}>Read More</Link>
          </Box>
